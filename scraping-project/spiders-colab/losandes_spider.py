...
class LosAndesScraper:
    ...
    def scrape_all_news(self, max_workers=5):
    ...
    def save_to_csv(self, filename='losandes_noticias.csv'):
    ...
    def save_to_json(self, filename='losandes_noticias.json'):
...
def main():
    ...
    scraper.scrape_all_news(max_workers=3)  # Reducir workers para ser más respetuoso
    
    # Guardar los resultados
    scraper.save_to_csv('losandes_noticias_completas.csv')
    scraper.save_to_json('losandes_noticias_completas.json')
    
    print(f"🎯 SCRAPING COMPLETADO")
    print(f"📊 Total de noticias extraídas: {len(scraper.all_news)}")
    print(f"📁 Archivos generados:")
    print(f"  - losandes_noticias_completas.csv")
    print(f"  - losandes_noticias_completas.json")
