#!/usr/bin/env python3
"""
Script para migrar datos de Los Andes desde CSV/JSON a PostgreSQL
"""
...
def find_latest_files():
    """Encontrar los archivos más recientes de Los Andes"""
    data_folder = "data/losandes"
    ...
    # Buscar archivos CSV
    csv_files = glob.glob(f"{data_folder}/losandes_*.csv")
    json_files = glob.glob(f"{data_folder}/losandes_*.json")
    
    if not csv_files and not json_files:
        print(f"❌ No se encontraron archivos de Los Andes en {data_folder}")
        return None, None
...
def main():
    """Función principal"""
    print("🔄 Migrador de Los Andes a PostgreSQL")
    print("=" * 50)
    ...
    if not csv_file and not json_file:
        ...
        print("💡 Ejecuta primero: python spiders/losandes_local.py")
        return
