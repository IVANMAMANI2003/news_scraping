#!/usr/bin/env python3
"""
Script para migrar datos de Sin Fronteras desde CSV/JSON a PostgreSQL
"""
...
def find_latest_files():
    """Encontrar los archivos más recientes de Sin Fronteras"""
    data_folder = "data/sinfronteras"
    ...
    # Buscar archivos CSV
    csv_files = glob.glob(f"{data_folder}/sinfronteras_*.csv")
    json_files = glob.glob(f"{data_folder}/sinfronteras_*.json")
    
    if not csv_files and not json_files:
        print(f"❌ No se encontraron archivos de Sin Fronteras en {data_folder}")
        return None, None
...
def main():
    """Función principal"""
    print("🔄 Migrador de Sin Fronteras a PostgreSQL")
    print("=" * 50)
    ...
    if not csv_file and not json_file:
        ...
        print("💡 Ejecuta primero: python spiders/sinfronteras_local.py")
        return
