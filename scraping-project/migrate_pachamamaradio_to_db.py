#!/usr/bin/env python3
"""
Script para migrar datos de Pachamama Radio desde CSV/JSON a PostgreSQL
"""
        ...
        for index, row in df.iterrows():
            try:
                ...
                values = (
                    ...
                    row.get('imagenes'),
                    row.get('fuente', 'Pachamama Radio')
                )
        ...
        for item in data:
            try:
                ...
                values = (
                    ...
                    item.get('imagenes'),
                    item.get('fuente', 'Pachamama Radio')
                )
...
def find_latest_files():
    """Encontrar los archivos más recientes de Pachamama Radio"""
    data_folder = "data/pachamamaradio"
    ...
    # Buscar archivos CSV
    csv_files = glob.glob(f"{data_folder}/pachamamaradio_*.csv")
    json_files = glob.glob(f"{data_folder}/pachamamaradio_*.json")
    
    if not csv_files and not json_files:
        print(f"❌ No se encontraron archivos de Pachamama Radio en {data_folder}")
        return None, None
...
def main():
    """Función principal"""
    print("🔄 Migrador de Pachamama Radio a PostgreSQL")
    print("=" * 50)
    ...
    if not csv_file and not json_file:
        ...
        print("💡 Ejecuta primero: python spiders/pachamamaradio_local.py")
        return
