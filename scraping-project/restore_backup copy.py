...
def restore_backup():
    ...
    try:
        ...
        # Eliminar duplicados por URL (mantener el registro con mayor ID)
        print("\n   🧹 Eliminando duplicados por URL (manteniendo el más reciente)...")
        cursor.execute("""
        ...
        if deleted_noticias > 0:
            print(f"      - Eliminados {deleted_noticias} registros duplicados por URL en 'noticias'")
        if deleted_social > 0:
            print(f"      - Eliminados {deleted_social} registros duplicados por URL en 'social_news'")
